...
- 오류당 최대 <N>회. 한 Retry에는 원인 하나와 최소 파일만
- 중단 조건: 동일 fingerprint <N>회 / 같은 검증 연속 실패 / 총 시도 상한 초과 / 원인이 스펙·권한·데이터 구조 판단으로 이동 / 관련 파일 특정 실패
- 중단 후: 로그 <M>줄 요약 → 원인 후보 2~3개 → task 축소 → **새 컨텍스트에서 재시작**
...
- 모델에 넘기는 로그 상한: <M>줄 요약. 전체 로그·코드베이스·대화 기록은 넘기지 않음
- 고성능 모델 호출 횟수: <단계별 상한>
- 중단 조건에 걸리면 추가 모델 호출 없음
