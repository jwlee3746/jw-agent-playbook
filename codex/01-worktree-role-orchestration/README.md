...
**출처**: 실습 — <멀티 에이전트 커맨드 센터> 교육 과정. starter 프로젝트와 이론 자료가 제공됨
**상태**: 실험 중
**포팅**: 없음. Claude Code에도 옮길 수 있으나 아직 안 해봄

이 항목은 **설계와 재현 절차만 기록한다.** starter 코드와 과제 데이터는 담지 않는다. 교육 과정에서 받은 자료라 재배포 조건이 확인되지 않았고, 재현에 필요한 것은 아래 구조이지 애플리케이션 코드가 아니다.
...
강의가 제시한 "좋은 작업 지시"의 구성이다. 위 `acceptance.json`은 이 중 범위·허용/금지·검증을 데이터로 옮긴 형태다.
...
실습의 기본 경로다. 이때는 **손으로 `git worktree` 명령을 실행하지 않는다.**
