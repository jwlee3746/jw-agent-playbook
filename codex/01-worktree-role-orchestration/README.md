...
역할마다 이 골격으로 `tasks/<task-id>-<역할>.md`를 하나씩 쓰고, 세션 시작 시 그 파일을 준다. `<...>` 자리를 채워 쓴다.
...
## 중단 조건
<언제 멈추고 사람을 부르는가 — 예: 검증 실패가 연속 3회 반복될 때>
...
**역할마다 금지 항목이 다르다.** 위 골격은 공통이고, 아래를 역할별로 덧붙인다.

| 역할 | 추가로 넣을 것 |
|---|---|
| 구현(Backend/Frontend) | 다른 역할의 구현 파일에 손대지 않는다 |
| Integration | 구현 파일 수정 금지. 충돌이 나면 임의로 해소하지 말고 멈춘다 |
| Tester | 구현 코드·검증 스크립트·채점 파일 모두 수정 금지. **통과시키려고 결과를 조작하지 않는다.** 시작 전 working tree가 clean한지 먼저 확인한다. `status`는 항상 `pending` |
