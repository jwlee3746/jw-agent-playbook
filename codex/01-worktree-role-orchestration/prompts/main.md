...
7. 최종 리뷰
   ...
   - 사람이 짧은 시간에 판단할 수 있도록 한곳에 모아 제시한다:
     각 역할 상태, 변경 파일 목록, 검증 결과와 exit code, 위험 항목, 승인 상태
   - 허용된 파일 안에서 요청 범위를 넘은 변경이 있는지 짚는다
     (감사 스크립트는 파일 단위까지만 판정한다)
   - approval draft 의 status 는 항상 pending. recommendation 은 참고만
8. 사람 승인 전에는 merge 하지 않는다

## 실패가 반복될 때
같은 역할이 같은 실패를 두 번 내면 세 번째 시도를 시키지 않는다.
1. 멈춘다
2. 실패 로그를 20줄 이내로 요약한다
3. 원인 후보를 2~3개로 나눈다
4. 그 역할의 task 를 더 작게 다시 쓴다
5. 새 컨텍스트에서 다시 시작한다
같은 task 를 그대로 새 세션에 던지지 않는다.
